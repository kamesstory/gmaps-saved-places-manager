    ...
    try {
      ...
      for (const listName of lists) {
        try {
          ...
          stats.errors.push({ list: listName, phase: 'pull', error: error.message });
        }
      }
      ...
      for (const [listName, remotePlaces] of Object.entries(remoteState)) {
        try {
          ...
          const changes = this.changeDetector.detectChangesForList(listName, remotePlaces);
          ...
          console.log(`  Notes: ${changes.notesChanges.length}, Associations: ${changes.associationChanges.length}, Conflicts: ${changes.conflicts.length}`);
        } catch (error) {
          console.error(`  ❌ Error detecting changes for ${listName}:`, error.message);
          stats.errors.push({ list: listName, phase: 'detect', error: error.message });
        }
      }
